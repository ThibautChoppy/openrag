...
docker compose \
    ...
    --entrypoint "uv run alembic -c /app/openrag/services/persistence/migrations/alembic.ini revision --autogenerate -m '<revision_message>'" \
    openrag
```

It will create a new migration script in the `openrag/services/persistence/migrations/versions/` directory.
...
docker compose \
    run --no-deps --build --rm \
    --entrypoint "uv run alembic -c /app/openrag/services/persistence/migrations/alembic.ini upgrade head" \
...
docker compose exec openrag \
    uv run alembic -c /app/openrag/services/persistence/migrations/alembic.ini upgrade head
```
...
DATABASE_URL=postgresql://user:pass@localhost:5432/openrag \
    uv run alembic -c openrag/services/persistence/migrations/alembic.ini upgrade head
```
...
docker compose exec -w /app/openrag openrag \
  /app/.venv/bin/alembic -c services/persistence/migrations/alembic.ini upgrade head
```
:::
...
1. **Create a merge migration** to join the two heads. Replace the revision IDs below with your actual head revisions:
     ...
     docker compose run --no-deps --build --rm \
         --entrypoint "uv run alembic -c /app/openrag/services/persistence/migrations/alembic.ini merge -m 'merge heads' <head1> <head2>" \
         openrag
     ```
